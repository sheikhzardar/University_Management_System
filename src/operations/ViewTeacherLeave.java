...
public class ViewTeacherLeave implements ActionListener 
{
	...
	{
		frame = new JFrame("Teacher's Leave Details");
		frame.setBounds(300, 80, 1000, 650);
		frame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		frame.setResizable(false);
		
		lblheading = new JLabel("Teacher's Leave Details");
		...
		{
			con=new Connect().connect();
			String statement="SELECT * FROM leaves_teacher";
			PreparedStatement pstmt=con.prepareStatement(statement);
			ResultSet rs=pstmt.executeQuery();
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource()==btnSearch)
		{
			...
			try 
			{
				...
				con=new Connect().connect();
				PreparedStatement pstmt = con.prepareStatement("SELECT * FROM leaves_teacher where id = '"+roll+"'");
				ResultSet rs = pstmt.executeQuery();
				table.setModel(DbUtils.resultSetToTableModel(rs));
			}
			catch(Exception e)
